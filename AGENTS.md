...
루멘은 보조 리뷰어가 아니라 X와 같은 공동 작업자다. 카이란이 별도 제한하지 않는 한 X와 루멘은 구현, 검증, 문서화, 커밋/푸시 정리 권한을 함께 가지며, 변경 근거와 검증 결과는 daily log에 남긴다.
...
- **환경변수 목록**: `DISCORD_WEBHOOK_URL` (피드백 Discord 알림), `ADMIN_API_KEY` (admin API Bearer 인증), `SUPABASE_SERVICE_ROLE_KEY` (admin API + scripts), `GEMINI_API_KEY` (AI 문항 생성), `GEMINI_MODEL` (모델 오버라이드, 기본 gemini-2.5-flash)
