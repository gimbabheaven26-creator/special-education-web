...
src/lib/db/__tests__/sync-all-stores.test.ts  # syncAllStores 부분 실패 hydration + Zod 손상 데이터 거부 회귀 (V 권고)
src/components/__tests__/SyncManager.test.tsx  # syncOnLogin 뮤텍스 회귀 (SyncManager 첫 테스트, V 권고)
...
scripts/audit-content-gap.mjs       # 퀴즈 과목별 분포 감사 + KICE 갭 측정
scripts/batch-generate.mjs          # M4 AI 배치 문항 생성 (--subject/--type/--count/--config/--dry-run)
...
- **환경변수 목록**: `DISCORD_WEBHOOK_URL` (피드백 Discord 알림), `ADMIN_API_KEY` (admin API Bearer 인증), `SUPABASE_SERVICE_ROLE_KEY` (admin API + scripts), `GEMINI_API_KEY` (AI 문항 생성), `GEMINI_MODEL` (모델 오버라이드, 기본 gemini-2.5-flash), `NEXT_PUBLIC_SENTRY_DSN` (Sentry 에러 모니터링, 없으면 SDK no-op)
- **Vercel 전용 환경변수**: `SENTRY_AUTH_TOKEN`, `SENTRY_ORG` (427d7de1d527), `SENTRY_PROJECT` (javascript-nextjs) — Source Map 업로드용
...
- **Sentry 에러 모니터링**: 3중 캡처 — client(`sentry.client.config.ts`), server+edge(`src/instrumentation.ts` onRequestError), global(`global-error.tsx`). API 라우트 13개 `captureException` 직접 호출. `enabled: !!process.env.NEXT_PUBLIC_SENTRY_DSN` — env 없으면 no-op
...
M1~M3 완료. M4 Phase 0+1 완료, Phase 2 미실행. M5(구조 재구성) 폐기 — SEW Next 프로토타입 삭제, 기존 48개 라우트 유지. M5.1: 안정성·품질·연계성 — Phase 1~3 전체 완료, **Completion Contract V 검증 22/22 PASS로 마일스톤 종료 (2026-06-11)**. V 권고 회귀 테스트 3건 이행 + stale E2E 제거로 CI 복구 완료 (2026-06-12). 잔여 V LOW 2건(CompletionScreen 조건부 넛지, flush keepalive)은 차기 작업 후보. 상세는 `prompt_plan.md` 참조.
