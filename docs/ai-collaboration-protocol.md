...
| X | Claude Code | 공동 실행자. 구현, 데이터 파이프라인, UI, 긴 맥락 유지 |
| 루멘 | Codex | 공동 실행자. 구현, 검증, 다른 모델 시각의 리뷰, 구조적 맹점 탐지, 설계 반박, rescue |
...
X와 루멘은 같은 repo에서 함께 작업하는 공동 실행자다. 카이란이 특정 작업에서 별도 제한을 두지 않는 한 두 주체는 구현, 검증, 문서화, 커밋/푸시 정리까지 같은 작업 권한을 가진다.

공동 권한의 규칙:

- X와 루멘은 직접 코드 수정, 빌드/테스트 실행, 문서 업데이트, 커밋/푸시 정리를 수행할 수 있다.
- 작업 시작 전 `git status --short --untracked-files=all`로 다른 주체의 변경 묶음을 확인하고, unrelated 변경은 되돌리지 않는다.
- 중요한 판단은 작업 중 짧게 통보하고, daily log에는 변경 파일, 검증 명령, exit code, 다음 액션을 남긴다.
- `docs/contract.md`, RLS, service role key, 데이터 정합성 위험은 V-ESCALATE로 넘긴다. 루멘도 수정 권한은 가지지만, 해당 영역의 최종 검증 판정은 V가 맡는다.
- 커밋이 필요하면 커밋 메시지나 daily log에 `X`, `lumen`, `lumen-led`, `co-authored` 등 실제 작업 주체를 드러낸다.
- X↔루멘 전환은 daily log와 git diff를 기준으로 이어받는다. 대화 transcript 공유를 전제로 삼지 않는다.
...
카이란이 별도 Codex 앱/세션에서 루멘과 대화하거나, 루멘이 독립 세션에서 직접 구현/검증을 이어간다. 결과는 파일, git diff, daily log로 넘긴다.

Claude Code를 쓰지 않는 기간, X가 토큰 제한으로 멈춘 기간, 또는 카이란이 루멘에게 직접 작업을 맡긴 기간에는 이 비동기 경로를 기본으로 한다.
...
| 핵심 질문 | "이 코드가 계약을 지키는가?" | "이 구현/설계가 실제로 견고한가?" |
| 검증 대상 | contract.md 준수, 보안, RLS, 데이터 정합성 | 구현 품질, 아키텍처, 공급망, 충돌 방지, 크로스 모델 편향 |
| 실행 방식 | Agent(subagent_type: "v") | `/codex:review` 또는 독립 Codex 세션 |
| 결과 저장 | `docs/v-reviews/` | `docs/reviews/lumen/` |

둘 다 쓰는 경우: 중요 변경 시 V가 contract 준수를 검증하고, 루멘이 구현/구조 크로스체크를 병렬로 수행.
...
1. X 또는 루멘이 `git status --short --untracked-files=all`로 현재 변경 묶음을 확인한다.
2. X 또는 루멘이 구현한다.
3. 작업 주체가 local daily log에 작업 요약과 검증 결과를 남긴다.
4. 다음 주체에게 넘길 내용이 있으면 `channel.md`에 지니 앞으로 짧은 handoff를 남긴다.
5. 크로스체크가 필요하면 X가 `/codex:adversarial-review`를 실행하거나 루멘이 독립 Codex 세션에서 리뷰한다.
6. 루멘 리뷰 결과가 있으면 `docs/reviews/lumen/` 또는 daily log에 저장한다.
7. X 또는 루멘이 수정한다.
8. 중요 변경이면 V가 contract 검증을 추가로 수행한다.
9. `npm run build` exit 0 확인 후 X 또는 루멘이 커밋/푸시를 정리한다.
10. phase 분기점 또는 큰 패치라면 Notion 스프린트 로그에 외부 기록을 남긴다.
...
- X와 루멘은 모두 커밋/푸시를 정리할 수 있다. 단, 커밋 전 `npm run build` exit 0 확인은 동일하게 필수다.
- 루멘 rescue가 패치를 만든 경우 리뷰 파일이나 daily log에 `FIXED by Lumen rescue (commit <hash>)`, `FIXED by X after Lumen review (commit <hash>)`처럼 주체와 승인 경로를 남긴다.
- 커밋 메시지는 가능하면 `docs(X):`, `docs(lumen):`, `fix(X):`, `fix(lumen):`, `fix(co-authored):`처럼 주체를 드러낸다. git author가 동일하므로 메시지와 daily log가 감사 추적 수단이다.
- X와 루멘은 같은 파일을 동시에 수정하지 않는다.
- 루멘은 기본적으로 공동 작업 권한을 가진다. 단, 사용자가 read-only 리뷰를 명시한 세션에서는 review-only를 지킨다.
- 루멘이 직접 수정하는 rescue는 공동 작업 권한의 일부로 수행할 수 있다. 단, 사용자가 read-only를 명시한 세션에서는 조사와 제안까지만 한다.
