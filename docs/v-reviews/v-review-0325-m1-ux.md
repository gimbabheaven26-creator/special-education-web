...
| 4 | SyncManager — schedulePush 클로저가 stale userIdRef 참조 가능 | LOW | 강선생1 | WONTFIX (실제 버그 아님, ref 기반으로 stale 불가) |
| 5 | sw.js — /api/ GET 경로 캐싱 활성화 상태 (기존 감시 항목 재확인) | MEDIUM | 강선생1 | FIXED (1bc28c8) |
| 6 | EmptyState — action 링크에 aria-label 없음 | LOW | 강선생1 | FIXED (ariaLabel prop 이미 존재) |
| 7 | BottomTabBar scrollToTop — SPA 탐색 시 smooth 옵션 미적용 (UX) | LOW | 강선생1 | FIXED (behavior: 'smooth' 이미 적용) |
