...
import type { CurriculumLevel, AchievementPool } from '@/types/achievement-standards';
...
역할:
- 2022 개정 기본교육과정 성취기준에 기반하여 주차별 수업 계획을 생성합니다.
- 학생의 장애 특성, 현행수준(3축 평가), 목표 수준을 고려합니다.
- 교육과정 성취수준(지식·이해, 과정·기능, 가치·태도)과 고려사항을 반영합니다.
...
6. 현행수준 정보가 제공되면, 학생의 현재 수준에서 목표 수준까지 점진적으로 도달하도록 설계하세요.
7. 고려사항이 제공되면 반드시 수업 활동에 반영하세요.
8. 반드시 순수 JSON만 출력하세요. 마크다운 코드블록(\`\`\`), 설명, 주석을 절대 포함하지 마세요. 첫 글자가 { 이어야 합니다.`;
}
...
export function buildUserPrompt(input: GenerationInput): string {
  const goalsText = input.goals
    .map((g, i) => {
      ...
      if (g.considerations && g.considerations.length > 0) {
        lines.push(`  - 고려사항: ${g.considerations.join('; ')}`);
      }

      if (g.curriculum_levels && g.curriculum_levels.length > 0) {
        const topLevel = g.curriculum_levels[0];
        lines.push(
          `  - 교육과정 최고수준: 지식·이해(${topLevel.knowledge_understanding}), 과정·기능(${topLevel.process_skills}), 가치·태도(${topLevel.values_attitudes})`
        );
      }
