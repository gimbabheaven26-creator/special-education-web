# M2: 만족도 끌어올리기 — 엔진→바퀴 연결

> 작성: 2026-04-06 | 담당: X | 상태: M2 전체 완료 (2026-04-20)
> 배경: M1(기술 구조)+M3(갭 해소) 완료. 카이란 자가평가 4영역 전부 미달
> (실력쌓기 30%, 내기록 20%, 함께하기 5%, 진단평가 30%).
> 기능은 있지만 깊이가 부족하다. "엔진은 돌아가는데 바퀴에 동력이 전달되지 않는다."

## 카이란 확정 결정 (2026-04-06)

1. **함께하기**: 키운다. 커뮤니티 재설계 진행
2. **콘텐츠 생성**: AI+사람 인터랙티브 검수 시스템 먼저 구축. AI 임의 생성 금지
3. **Quiz Editor**: 다른 세션에서 작업 중 → M2 제외
4. **scripts 정리**: ~~다른 세션에서 작업 중~~ B-1/B-2/B-3 전체 완료 (c2f3e2f, 2026-04-06)
5. **scenario SQL**: 수정 중 → 카이란 확인 대기

## Phase 1: 기출→개념 직링크 (실력쌓기 핵심)

**목표**: /kice 기출문제에서 관련 개념 페이지로 직링크. 3층 시스템의 바퀴 연결.

**변경 파일**: `src/app/kice/KiceClient.tsx`, `src/lib/kice/keyword-concept-map.ts`, `src/lib/content/concept-urls.ts`, `src/components/kice/QuestionCard.tsx`, `src/app/concepts/[subject]/[slug]/page.tsx`

- [x] kice-terms.json 1417개 키워드 → concepts/ 101개 MDX 매칭 추출
- [x] 기출문제 카드 하단에 "관련 개념" 링크 1~3개 (BookOpen 아이콘)
- [x] concepts/[subject] 페이지에 "이 개념의 기출" 역링크 (연도+문번)
- [x] 매핑 없는 기출은 링크 없이 정상 표시

## Phase 2: 진단→학습 경로 연결 (진단평가 핵심)

**목표**: 진단 결과가 구체적 다음 행동으로 이어진다.

**변경 파일**: `src/app/diagnosis/` 관련, `src/components/dashboard/` 위젯

- [x] 진단 결과 요약에 "추천 학습 경로" TOP 3 추가 (개념 + 퀴즈 + 기출 링크)
- [x] 약점 없을 때 격려 메시지 표시 ("모든 영역을 고르게 학습하고 있어요")
- [x] 홈 대시보드에 DiagnosticRecommendCard 표시 (약점 1위 과목 + 개념/퀴즈/기출 + 집중과목 설정)
- [x] 진단 완료 시 최약점 과목(60% 미만) → useFocusStore 자동 반영 (사용자 직접 설정 시 미변경)
- [x] 진단 데이터 0건 시 "진단평가 시작하기" CTA 표시

## Phase 3: 함께하기 재설계 (5% → 30%+)

**목표**: "SNS"가 아니라 "함께 공부하는 느낌". 커뮤니티 가치 재정의.

**변경 파일**: `src/app/community/` 관련

- [x] 커뮤니티 현재 기능 전수 파악 (AI 문제, 질문, 투표)
- [x] "오늘의 문제" 공유 (매일 1문제 전체 사용자 동시 풀기 + 정답률)
- [x] "나도 틀렸어요" 공감 (오답노트에서 같은 문제 틀린 사람 수)
- [x] 학습 랭킹 옵트인 (주간 XP 상위 표시, 참여 선택제)

## Phase 4: 내기록 깊이 강화 (20% → 50%+)

**목표**: 숫자 나열이 아니라 "성장이 보이는" 대시보드.

**변경 파일**: `src/app/record/RecordDashboard.tsx`, `src/app/my/` 관련

- [x] 과목별 정답률 추이 그래프 (주간/월간 시각화)
- [x] "이번 주 성장 포인트" (지난주 대비 개선 영역 하이라이트)
- [x] 약점→강점 전환 기록 (과거 약했던 영역 개선 이력)

## Phase 5: AI-Human 문제 생성 파이프라인

**목표**: AI가 임의로 만들지 않는다. AI 초안 → 사람 검토/수정 → 확정 → DB.
기출/시나리오 같은 긴 문제에 특히 적용.

**변경 파일**: 신규 `/app/admin/` 관련, API Route

- [x] 문제 생성 입력 UI (키워드/주제/유형 입력 → AI 초안 생성)
- [x] 검수 UI (AI 초안 편집/승인/거절)
- [x] 확정 → Supabase DB 삽입 파이프라인
- [x] 기출/시나리오형 긴 문제 전용 템플릿 (Phase 5-B: scenario_composite UI + ai_status DB 스키마)

## 실행 순서

```
Phase 1 (기출→개념) → Phase 2 (진단→경로) → Phase 4 (내기록)
                                              ↗
                    Phase 3 (함께하기)
                    Phase 5 (AI-Human 파이프라인, 병렬 가능)
```

## Completion Contract

V(평가자)가 이 기준으로 PASS/FAIL을 판정한다. 80% 이상 통과해야 PASS.

### 기출→개념 연결 (Phase 1) ✅ 2026-04-07
- [x] /kice 기출문제 카드에 관련 개념 링크가 1개 이상 표시된다
- [x] 개념 링크 클릭 시 해당 concepts/[subject] 페이지로 이동한다
- [x] concepts/[subject] 페이지에서 관련 기출 연도+문번이 표시된다
- [x] 매핑되는 개념이 없는 기출은 링크 없이 정상 표시된다

### 진단→경로 연결 (Phase 2) ✅ 2026-04-08
- [x] 진단 결과 화면에 "보강이 필요한 과목" TOP 3 섹션이 있다
- [x] 추천 경로에 개념 페이지 링크와 퀴즈 링크가 포함된다
- [x] 홈 대시보드에 DiagnosticRecommendCard 진단 기반 추천 카드가 표시된다
- [x] 진단 약점 과목 → useFocusStore 수동 설정 버튼 (자동 반영은 UX 혼란 방지로 미적용)

### 함께하기 재설계 (Phase 3) ✅ 2026-04-20
- [x] "오늘의 도전" TodayChallenge — KICE 기출 매일 1문제 공유 학습
- [x] 빈 상태(참여자 0명)에서도 3단계 안내 + CTA (기존 유지)
- [x] 커뮤니티 첫 화면에 "오늘의 도전" 최상단 배치 → 즉시 가치 전달
- [x] "나도 틀렸어요" — 오답노트 카드에 공감 카운터 표시 (wrong_note_stats)
- [x] 주간 학습 랭킹 — 옵트인 토글, TOP 20, 커뮤니티 페이지 배치

### 내기록 깊이 (Phase 4) ✅ 2026-04-09
- [x] 과목별 정답률 추이가 시각적으로 표시된다
- [x] "이번 주 성장" 또는 이전 대비 변화가 표시된다
- [x] 데이터 0건일 때 적절한 EmptyState가 있다

### AI-Human 파이프라인 (Phase 5-A) ✅ 2026-04-09
- [x] AI 초안 생성 기능이 동작한다 (키워드 입력 → 문제 초안)
- [x] 검수 UI에서 편집/승인/거절이 가능하다
- [x] 승인된 문제가 Supabase DB에 삽입된다

### 접근성/빌드 ✅ 2026-04-09
- [x] 새 버튼/링크에 aria-label이 있다
- [x] `npm run build` exit 0
- [x] `npm run lint` 경고 0건
- [x] 기존 917+ 테스트 전부 통과 + 신규 테스트 추가 (938)

---

## 이전 계획

### 플래시카드 전면 재설계 + Supabase 동기화 수복 (2026-03-31)
> 담당: X | 상태: Phase 0~5 전체 완료 (2026-04-01)
> 플래시카드 재설계 5 Phase, Supabase sync 키 수복, 917 테스트, 빌드 exit 0

### M3: 만족도 갭 해소 (2026-03-30)
> 담당: X | 상태: B1~B7 전체 완료

### M2: 사용자 체감 개선 (2026-03-29)
> 담당: X | 상태: M2 FAIL (8/12 = 66.7%) — 기능+UX 8/8 PASS, 만족도 0/4 FAIL

### 나다운 MVP (2026-03-27~29)
> 담당: X | 상태: Phase 0~5 + AI 전체 완료 (MVP 100%)
