...
import { Printer, Eye, RefreshCw, ArrowLeft, PlayCircle, List } from 'lucide-react';
...
function toWorksheetConfig(
  topicName: string,
  questions: WorksheetQuestionRow[],
): WorksheetConfig {
  const wQuestions: WorksheetQuestion[] = questions.map((q) => ({
    ...q,
    topicId: q.topic_id,
  }));

  const types = new Set(questions.map((q) => q.type));
  const type: WorksheetConfig['type'] =
    types.size > 1 ? 'mixed' : (questions[0]?.type ?? 'fill_in');

  return {
    id: questions[0]?.topic_id ?? '',
    subject: questions[0]?.subject ?? '',
    topicId: questions[0]?.topic_id ?? '',
    topicName,
    type,
...
function ModeToggle({
  ...
  return (
    <div className="flex border border-border rounded-lg overflow-hidden print:hidden" role="tablist">
      <button
        role="tab"
...
export default function WorksheetViewClient({
  topicName,
  questions,
  qrUrl,
}: WorksheetViewClientProps) {
  const [showAnswers, setShowAnswers] = useState(false);
  const [mode, setMode] = useState<ViewMode>('all');
  const [solverKey, setSolverKey] = useState(0);

  const worksheetConfig = useMemo(
    () => toWorksheetConfig(topicName, questions),
    [topicName, questions],
  );

  const subjectTitle = useMemo(
    () => getSubjectTitle(questions[0]?.subject ?? ''),
    [questions],
  );
    ...
    <div className="max-w-4xl mx-auto px-4 md:px-8 py-8">
      ...
      {mode === 'step' ? (
        <WorksheetSolver
          ...
          onRetry={handleRetry}
          onNewWorksheet={handleBackToList}
        />
      ) : (
        <>
